...
extension Footprint {
    class DefaultMemoryProvider : MemoryProvider {
        ...
        func provide(_ pressure: Footprint.Memory.State = .normal) -> Footprint.Memory {
            ...
            let used = kerr == KERN_SUCCESS ? Int(info.phys_footprint) : 0
            let compressed = kerr == KERN_SUCCESS ? Int(info.compressed) : 0
...
#else
            let remaining = kerr == KERN_SUCCESS ? Int(info.limit_bytes_remaining) : 0
#endif
            return Footprint.Memory(used: used, remaining: remaining, compressed: compressed, pressure: pressure)
        }
